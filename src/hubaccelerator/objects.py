...
class Finding:
    ...
    def fullMap (self):
        ...
        Items with isKey=True comprise the finding key, which is a set of 
        attributes that will uniquely identify the finding.

        Items with isUpdate=True can be altered using the update function.

        The keys= parameter is a list of keys that reflects the key/value nesting
        in the Security Hub finding dict. Thus, if you want to map a column
        to finding["Severity"]["Product"] you would specify the keys value of 
        ["Severity", "Product"]

        The dict-to-list (d2l) and list-to-dict (l2d) transforms can 
        - Use a callable to produce the desired value for this column
        - Use a fixed string to force the value for this column.

        The d2lParameters and l2dParameters parameters are used to pass 
        additional information to the d2l and l2d transforms.
        """
        map = FindingColumnMap([
            FindingColumn(columnName="Id", keys=["Id"], isKey=True),
            FindingColumn(columnName="ProductArn", keys=["ProductArn"], isKey=True), 
            #### BEGIN Updatable fields using securityhub:batch_update_findings()
            FindingColumn(columnName="Criticality", keys=["Criticality"], 
                isUpdatable=True, d2l=FindingActions.forceInteger,
                l2d=FindingActions.forceInteger),
            FindingColumn(columnName="Confidence", keys=["Confidence"], 
                isUpdatable=True, d2l=FindingActions.forceInteger,
                l2d=FindingActions.forceInteger),
            FindingColumn(columnName="NoteText", keys=["Note", "Text"],
                isUpdatable=True),
            FindingColumn(columnName="NoteUpdatedBy", keys=["Note", "UpdatedBy"],
                isUpdatable=True, d2l=FindingActions.noteUpdater,
                d2lParameters={"actor" : self.actor , "finding": self },
                l2d=FindingActions.noteUpdater,
                l2dParameters={"actor" : self.actor , "finding": self }),
            FindingColumn(columnName="CustomerOwner",
                keys=["UserDefinedFields", "Owner"], isUpdatable=True),
            FindingColumn(columnName="CustomerIssue", 
                keys=["UserDefinedFields", "Issue"], isUpdatable=True),
            FindingColumn(columnName="CustomerTicket",
                keys=["UserDefinedFields", "Ticket"], isUpdatable=True),
            FindingColumn(columnName="ProductSeverity", 
                keys=["Severity", "Product"], isUpdatable=True,
                d2l=FindingActions.checkSeverity, 
                l2d=FindingActions.checkSeverity),
            FindingColumn(columnName="NormalizedSeverity",
                keys=["Severity", "Normalized"], isUpdatable=True,
                d2l=FindingActions.checkSeverity,
                l2d=FindingActions.checkSeverity),
            FindingColumn(columnName="SeverityLabel",
                keys=["Severity", "Label"], isUpdatable=True,
                d2l=FindingActions.checkSeverityLabel,
                l2d=FindingActions.checkSeverityLabel),
            FindingColumn(columnName="VerificationState", 
                keys=["VerificationState"], isUpdatable=True,
                d2l=FindingActions.checkVerificationState,
                l2d=FindingActions.checkVerificationState),
            FindingColumn(columnName="Workflow", keys=["Workflow", "Status"],
                isUpdatable=True, d2l=FindingActions.checkWorkflow,
                l2d=FindingActions.checkWorkflow),
            #### END Updatable fields using securityhub:batch_update_findings()
            FindingColumn(columnName="UpdateVersion", keys=[],   
                d2l=str(_CURRENT_VERSION), l2d=str(_CURRENT_VERSION)),
            FindingColumn(columnName="GeneratorId", keys=["GeneratorId"]),
            FindingColumn(columnName="AwsAccountId",keys=["AwsAccountId"]),
            FindingColumn(columnName="Types", keys=["Types"], 
                d2l=FindingActions.delist) ,
            FindingColumn(columnName="FirstObservedAt",keys=["FirstObservedAt"]),
            FindingColumn(columnName="LastObservedAt",keys=["LastObservedAt"]),
            FindingColumn(columnName="CreatedAt",keys=["CreatedAt"]),
            FindingColumn(columnName="UpdatedAt",keys=["UpdatedAt"]),
            FindingColumn(columnName="Title", keys=["Title"]),
            FindingColumn(columnName="Description", keys=["Description"]),
            FindingColumn(columnName="StandardsArn",
                keys=["ProductFields", "StandardsArn"]),
            FindingColumn(columnName="StandardsSubscriptionArn",
                keys=["ProductFields", "StandardsSubscriptionArn"]),
            FindingColumn(columnName="ControlId",
                keys=["ProductFields", "ControlId"]),
            FindingColumn(columnName="RecommendationUrl",
                keys=["ProductFields", "RecommendationUrl"]),
            FindingColumn(columnName="StandardsControlArn",
                keys=["ProductFields", "StandardsControlArn"]),
            FindingColumn(columnName="ProductName",
                keys=["ProductFields", "aws/securityhub/ProductName"]),
            FindingColumn(columnName="CompanyName",
                keys=["ProductFields", "aws/securityhub/CompanyName"]),
            FindingColumn(columnName="Annotation",
                keys=["ProductFields", "aws/securityhub/annotation"]),
            FindingColumn(columnName="FindingId",
                keys=["ProductFields", "aws/securityhub/FindingId"]),
            FindingColumn(columnName="Resources", keys=["Resources"], 
                d2l=FindingActions.resources),
            FindingColumn(columnName="ComplianceStatus",
                keys=["Compliance", "Status"]),
            FindingColumn(columnName="WorkflowState",keys=["WorkflowState"]),
            FindingColumn(columnName="RecordState",keys=["RecordState"])
        ])
...
class HubActor (Actor):
    ...
    #---------------------------------------------------------------------------
    @staticmethod
    def discoverAggregator(region: str = None, role: str = None) -> dict:
        """
        Auto-discover the Security Hub aggregation region and linked regions
        using list_finding_aggregators / get_finding_aggregator.

        Returns:
            {
                'aggregationRegion': str,    # the home/primary region
                'regions': list[str],        # all regions to scan
                'linkingMode': str,          # ALL_REGIONS, etc.
            }
        Or None if no aggregator is configured (single-region mode).

        Can be called before constructing a HubActor — uses a temporary
        session to query any region.
        """
        try:
            if role:
                ...
                creds = sts.assume_role(
                    RoleArn=role,
                    RoleSessionName='hubaccelerator-discover'
                )['Credentials']
            ...
            # List aggregators (currently returns at most one)
            aggregators = client.list_finding_aggregators()
            agg_list = aggregators.get('FindingAggregators', [])

            if not agg_list:
                _LOGGER.info('no finding aggregator configured — single-region mode')
                return None

            # Get the aggregator details
            agg_arn = agg_list[0]['FindingAggregatorArn']
            detail = client.get_finding_aggregator(FindingAggregatorArn=agg_arn)

            aggregation_region = detail.get('FindingAggregationRegion')
            linking_mode = detail.get('RegionLinkingMode', '')
            specified_regions = detail.get('Regions', [])

            # Determine the full region list based on linking mode
            if 'ALL_REGIONS' in linking_mode:
                # All regions (optionally minus exclusions)
                all_regions = session.get_available_regions('securityhub')
                if 'EXCEPT' in linking_mode and specified_regions:
                    regions = [r for r in all_regions if r not in specified_regions]
                else:
                    regions = all_regions
            else:
                # Only specified regions + the aggregation region
                regions = list(set([aggregation_region] + specified_regions))

            _LOGGER.info(f'discovered aggregator: home={aggregation_region}, '
                f'mode={linking_mode}, {len(regions)} region(s)')

            return {
                'aggregationRegion': aggregation_region,
                'regions': regions,
                'linkingMode': linking_mode,
            }

        except Exception as thrown:
            _LOGGER.warning(f'could not discover aggregator: {thrown}')
            return None
    #---------------------------------------------------------------------------
